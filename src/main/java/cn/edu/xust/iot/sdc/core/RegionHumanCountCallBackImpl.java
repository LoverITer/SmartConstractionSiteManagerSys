...
public class RegionHumanCountCallBackImpl implements HWPuSDK.pfRealDataCallBack {
    ...
    private CameraMapper cameraMapper=null;
    private IRegionHumanCountService regionHumanCountService=null;
    ...
    public Pointer invoke(Pointer szBuffer, NativeLong lSize, Pointer pUsrData) {
        ...
        if(cameraMapper==null){
            cameraMapper= ApplicationContextHolder.getBean("cameraMapper");
        }
        if(regionHumanCountService==null){
            regionHumanCountService=ApplicationContextHolder.getBean("regionHumanCountService");
        }
        ...
        if (isOk) {
            try {
                ...
                data.read();
                procMetaData(data, ip); // Resolving metadata
     ...
     * 处理实时视频智能元数据
     */
    private void procMetaData(HWPuSDK.PU_META_DATA metaData, String ip) throws Exception {
